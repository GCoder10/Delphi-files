object fInfoBox: TfInfoBox
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Kiermasz - komunikat'
  ClientHeight = 179
  ClientWidth = 595
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object gpInfoBox: TGridPanel
    Left = 0
    Top = 0
    Width = 595
    Height = 179
    Align = alClient
    Color = clSkyBlue
    ColumnCollection = <
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end>
    ControlCollection = <
      item
        Column = 2
        Control = fInfoBoxImage
        Row = 0
        RowSpan = 3
      end
      item
        Column = 3
        ColumnSpan = 4
        Control = PleaseWaitCxLabel
        Row = 0
        RowSpan = 3
      end
      item
        Column = 0
        ColumnSpan = 10
        Control = MessageContentCxLabel
        Row = 4
        RowSpan = 2
      end
      item
        Column = 7
        Control = fInfoBoxImage2
        Row = 0
        RowSpan = 3
      end
      item
        Column = 0
        ColumnSpan = 10
        Control = pbFInfoBox
        Row = 7
        RowSpan = 2
      end
      item
        Column = 4
        ColumnSpan = 2
        Control = cxProgressBarLabel
        Row = 9
      end>
    ParentBackground = False
    RowCollection = <
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end>
    TabOrder = 0
    object fInfoBoxImage: TImage
      Left = 119
      Top = 1
      Width = 59
      Height = 51
      Align = alClient
      Center = True
      Picture.Data = {
        
        F307D50000000049454E44AE426082}
      ExplicitLeft = 208
      ExplicitTop = 112
      ExplicitWidth = 105
      ExplicitHeight = 105
    end
    object PleaseWaitCxLabel: TcxLabel
      Left = 178
      Top = 1
      Align = alClient
      Caption = 'PROSZ'#280' CZEKA'#262
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -27
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taCenter
      Properties.Alignment.Vert = taVCenter
      AnchorX = 296
      AnchorY = 27
    end
    object MessageContentCxLabel: TcxLabel
      Left = 1
      Top = 69
      Align = alClient
      Caption = 'Trwa operacja na danych. Mo'#380'e to chwilk'#281' zaj'#261#263'.'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -24
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taCenter
      Properties.Alignment.Vert = taVCenter
      AnchorX = 298
      AnchorY = 86
    end
    object fInfoBoxImage2: TImage
      Left = 414
      Top = 1
      Width = 59
      Height = 51
      Align = alClient
      Center = True
      Picture.Data = {
        
        F307D50000000049454E44AE426082}
      ExplicitLeft = 420
      ExplicitTop = -5
    end
    object pbFInfoBox: TProgressBar
      Left = 1
      Top = 120
      Width = 593
      Height = 34
      Align = alClient
      Style = pbstMarquee
      TabOrder = 2
    end
    object cxProgressBarLabel: TcxLabel
      Left = 237
      Top = 154
      Align = alClient
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -16
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taCenter
      Properties.Alignment.Vert = taVCenter
      Transparent = True
      ExplicitLeft = 1
      ExplicitTop = 1
      ExplicitWidth = 7
      ExplicitHeight = 17
      AnchorX = 296
      AnchorY = 166
    end
  end
end
